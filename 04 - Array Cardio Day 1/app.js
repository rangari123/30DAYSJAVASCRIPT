...
const sortArray1 = inventors.sort(function (first, second) {
  if (first.year > second.year) {
    return 1;
  }
  return -1;
});
console.table(sortArray1);

// using tearnary
// const sortArray2 = inventors.sort((a, b) => (a.year > b.year ? 1 : -1));
// console.table(sortArray2);
// -- -----------------------------------------------------------------------------

// Array.prototype.reduce()
// 4. How many years did all the inventors live all together?
const totalYear = inventors.reduce((totalyear, inventor) => {
  return totalyear + (inventor.passed - inventor.year);
}, 0);

console.log(`total year live is ${totalYear}`);
...
const sortName = people.sort((lastOne, nexttOne) => {
  const [alast, afirst] = lastOne.split(", ");
  const [Blast, bfirst] = nexttOne.split(", ");
...
const tranportation = data.reduce((obj, item) => {
  if (!obj[item]) {
    obj[item] = 0;
  }
  obj[item]++;
